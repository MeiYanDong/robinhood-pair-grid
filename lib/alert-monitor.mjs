...
export function assertMartingaleReadbackReport(report) {
  ...
  // A canonical transaction can leave the transaction-layer PENDING_<key>
  // status while the rotation waits for a safe target. Unconfirmed intent is
  // deliberately excluded; the normal nonce and NFT checks still apply below.
  const canonicalRotationWait = Boolean(
    report.pendingRotation &&
    typeof report.status === 'string' &&
    report.status.startsWith('PENDING_ROTATION_') &&
    Object.keys(report.transactions || {}).length > 0 &&
    Object.values(report.transactions).every((transaction) => transaction.status === 'CANONICAL_SUCCESS'),
  )
  if (
    !['BUY_LADDER_ACTIVE', 'MARTINGALE_ACTIVE', 'ROTATION_PENDING'].includes(report.status) &&
    !canonicalRotationWait
  ) {
  ...
  if (report.pendingRotation) {
    if (
      (report.status !== 'ROTATION_PENDING' && !canonicalRotationWait) ||
      ![4, 5].includes(readablePositions)
    ) {
