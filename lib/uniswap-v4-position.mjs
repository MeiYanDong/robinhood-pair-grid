import { decodeAbiParameters, encodeAbiParameters, encodeFunctionData, getAddress, keccak256 } from 'viem'
...
/**
 * Validate the successful raw return from the two-call PositionManager
 * multicall used by every permit-backed mint. The first inner result encodes
 * the bytes error returned by permitBatch; the second is the void
 * modifyLiquidities result.
 *
 * @param {import('viem').Hex} data
 */
export function validateBatchMintSimulationData(data) {
  if (typeof data !== 'string' || !/^0x[0-9a-f]*$/iu.test(data) || data === '0x') {
    throw new Error('批量 mint eth_call 缺少 multicall 返回数据')
  }
  let results
  try {
    ;[results] = decodeAbiParameters([{ name: 'results', type: 'bytes[]' }], data)
  } catch {
    throw new Error('批量 mint eth_call 返回数据无法按 multicall 解码')
  }
  if (results.length !== 2) throw new Error(`批量 mint multicall 返回 ${results.length} 项，预期 2 项`)
  let permitError
  try {
    ;[permitError] = decodeAbiParameters([{ name: 'err', type: 'bytes' }], results[0])
  } catch {
    throw new Error('批量 mint Permit2 返回数据无法解码')
  }
  if (permitError !== '0x') throw new Error('批量 mint Permit2 返回非空错误')
  if (results[1] !== '0x') throw new Error('批量 mint modifyLiquidities 返回异常数据')
  return true
}
...
/**
 * Mint several new positions in one PositionManager unlock. The pool settles
 * once after every MINT_POSITION action, so the wallet signs one bounded
 * Permit2 batch and pays one mint transaction for the complete ladder.
 *
 * @param {{poolKey: PoolKeyInput, positions: Array<{tickLower: number, tickUpper: number,
 * liquidity: bigint, amount0Max: bigint, amount1Max: bigint, recipient: string,
 * hookData?: import('viem').Hex}>, batchPermit: BatchPermit, deadline: bigint}} input
 */
export function encodeBatchMintPositions({ poolKey, positions, batchPermit, deadline }) {
  const normalizedPoolKey = normalizePoolKey(poolKey)
  if (!Array.isArray(positions) || positions.length < 2 || positions.length > 16) {
    throw new RangeError('批量 mint 头寸数必须介于 2 和 16')
  }
  const params = positions.map((position, index) => {
    assertTick(position.tickLower)
    assertTick(position.tickUpper)
    if (position.tickLower >= position.tickUpper) throw new RangeError(`第 ${index + 1} 档 tick 区间无效`)
    if (
      position.tickLower % normalizedPoolKey.tickSpacing !== 0 ||
      position.tickUpper % normalizedPoolKey.tickSpacing !== 0
    ) {
      throw new RangeError(`第 ${index + 1} 档 tick 未对齐 tickSpacing`)
    }
    assertBigInt(`第 ${index + 1} 档 liquidity`, position.liquidity, { positive: true, max: MAX_UINT128 })
    assertBigInt(`第 ${index + 1} 档 amount0Max`, position.amount0Max, { max: MAX_UINT128 })
    assertBigInt(`第 ${index + 1} 档 amount1Max`, position.amount1Max, { max: MAX_UINT128 })
    if (position.amount0Max === 0n && position.amount1Max === 0n) {
      throw new Error(`第 ${index + 1} 档没有可结算代币`)
    }
    return encodeParameters(MINT_PARAMETERS, [
      normalizedPoolKey,
      position.tickLower,
      position.tickUpper,
      position.liquidity,
      position.amount0Max,
      position.amount1Max,
      getAddress(position.recipient),
      position.hookData || '0x',
    ])
  })
  const actions = /** @type {number[]} */ (positions.map(() => ACTION.MINT_POSITION))
  actions.push(ACTION.SETTLE_PAIR)
  params.push(
    encodeParameters(SETTLE_PAIR_PARAMETERS, [normalizedPoolKey.currency0, normalizedPoolKey.currency1]),
  )
  return withBatchPermit(encodeModifyLiquidities(actions, params, deadline), batchPermit)
...
/**
 * Remove all liquidity and burn the now-empty position NFT. This is the
 * finite-state rotation primitive: every completed leg is replaced by one new
 * NFT, so unattended operation cannot accumulate an unbounded set of empty
 * position tokens.
 *
 * @param {{poolKey: PoolKeyInput, tokenId: bigint, amount0Min: bigint,
 * amount1Min: bigint, deadline: bigint, hookData?: import('viem').Hex}} input
 */
export function encodeBurnPosition({ poolKey, tokenId, amount0Min, amount1Min, deadline, hookData = '0x' }) {
  const normalizedPoolKey = normalizePoolKey(poolKey)
  assertBigInt('tokenId', tokenId)
  assertBigInt('amount0Min', amount0Min, { max: MAX_UINT128 })
  assertBigInt('amount1Min', amount1Min, { max: MAX_UINT128 })
  return encodeModifyLiquidities(
    [ACTION.BURN_POSITION, ACTION.TAKE_PAIR],
    [
      encodeParameters(BURN_PARAMETERS, [tokenId, amount0Min, amount1Min, hookData]),
      encodeParameters(TAKE_PAIR_PARAMETERS, [
        normalizedPoolKey.currency0,
        normalizedPoolKey.currency1,
        MSG_SENDER,
