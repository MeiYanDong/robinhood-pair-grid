 ...
 * metadata?:any, account:any, walletClient:any, publicClient:any, walletAddress:string,
...
export async function executePersistedTransaction(input) {
  ...
  if (!receipt) {
    ...
    if (!knownTransaction) {
      try {
        const sentHash = await input.walletClient.sendRawTransaction({ serializedTransaction: serialized })
        if (sentHash.toLowerCase() !== expectedHash.toLowerCase()) throw new Error('RPC 返回哈希不一致')
      } catch (error) {
        if (!/already known|known transaction|nonce too low/iu.test(errorMessage(error))) throw error
      }
    }
